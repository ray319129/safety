...
BMduino 透過序列埠（115200 baud）接收指令，指令格式為純文字，以 `\n` 結尾。
...
2. 設定：115200 baud，換行符號選擇「Newline」
3. 輸入測試指令：
   ```
   M F 50
   ```
...
- **檢查 USB 連接**：確認 USB Type-C 線已正確連接
- **檢查驅動程式**：Windows 可能需要安裝 CH340/CP2102 驅動
- **檢查埠號**：在裝置管理員（Windows）或 `ls /dev/tty*`（Linux）確認埠號

### 問題 2：指令無反應

- **檢查序列埠設定**：確認 baud rate 為 115200
- **檢查指令格式**：確認指令以 `\n` 結尾
- **檢查硬體連接**：確認所有腳位連接正確
...
4. **序列埠通訊**：
   - 確保 Raspberry Pi 和 BMduino 使用相同的 baud rate（115200）
   - 指令必須以 `\n` 結尾
